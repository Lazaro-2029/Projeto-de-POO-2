...
public class AvaliadorDAO {
    ...
    public Avaliador buscarPorCpf(String cpf) {
        String sql = "SELECT u.cpf, u.nome, u.endereco FROM avaliador av JOIN usuario u ON av.cpf_usuario=u.cpf WHERE av.cpf_usuario=?";
        try (PreparedStatement ps = con().prepareStatement(sql)) {
            ps.setString(1, cpf);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return new Avaliador(new Usuario(rs.getString("cpf"), rs.getString("nome"), rs.getString("endereco")));
        } catch (SQLException e) { e.printStackTrace(); }
        return null;
    }

    public Avaliador buscarPorNome(String nome) {
        String sql = "SELECT u.cpf, u.nome, u.endereco FROM avaliador av JOIN usuario u ON av.cpf_usuario=u.cpf WHERE u.nome=?";
        try (PreparedStatement ps = con().prepareStatement(sql)) {
            ps.setString(1, nome);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return new Avaliador(new Usuario(rs.getString("cpf"), rs.getString("nome"), rs.getString("endereco")));
    ...
    public List<Avaliador> listarTodos() {
        ...
        String sql = "SELECT u.cpf, u.nome, u.endereco FROM avaliador av JOIN usuario u ON av.cpf_usuario=u.cpf ORDER BY u.nome";
        try (PreparedStatement ps = con().prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) lista.add(new Avaliador(new Usuario(rs.getString("cpf"), rs.getString("nome"), rs.getString("endereco"))));
        } catch (SQLException e) { e.printStackTrace(); }
        return lista;
    }
