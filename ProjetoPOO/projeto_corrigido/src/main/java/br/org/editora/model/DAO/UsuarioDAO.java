...
public class UsuarioDAO {
    ...
    public Usuario inserir(Usuario u) {
        String sql = "INSERT INTO usuario (cpf, nome, endereco, senha_hash) VALUES (?, ?, ?, ?)";
    ...
    public Usuario buscarPorCpf(String cpf) {
        String sql = "SELECT cpf, nome, endereco, senha_hash FROM usuario WHERE cpf = ?";
        try (PreparedStatement ps = con().prepareStatement(sql)) {
            ps.setString(1, cpf);
    ...
    public Usuario buscarPorNome(String nome) {
        String sql = "SELECT cpf, nome, endereco, senha_hash FROM usuario WHERE nome = ?";
        try (PreparedStatement ps = con().prepareStatement(sql)) {
            ps.setString(1, nome);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return new Usuario(
    ...
    public List<Usuario> listarTodos() {
        ...
        String sql = "SELECT cpf, nome, endereco, senha_hash FROM usuario";
        try (PreparedStatement ps = con().prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
    ...
    public void atualizarSenha(String cpf, String novoHash) {
